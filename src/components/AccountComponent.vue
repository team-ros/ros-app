            ...
            <v-col class="col-12 mt-6">
                ...
                <div class="mt-6">
                    ...
                    <v-text-field
                        ...
                        v-model="user.email"
                    ></v-text-field>
                    <h1 style="font-size: large; margin-top: 30px;" class="mb-6">{{$t('site.account.reset_password')}}</h1>
                    <v-text-field
                        color="#0044b2"
                        type="email"
                        :label="$t('site.account.email')"
                        v-model="email"
                    ></v-text-field>
                    <v-btn @click="sendMail" block color="#0044b2" depressed style="color: #eeeeee"
                    >{{$t('site.account.reset_button')}}
                    </v-btn>
                </div>
            </v-col>
        </div>
        <v-snackbar v-model="sent" :timeout="2000" color="success">
          {{$t('site.account.email_sent')}}
        </v-snackbar>
